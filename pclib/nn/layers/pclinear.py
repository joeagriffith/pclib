...
class PPELayer(nn.Module):
    ...
    def forward(self, x, state) -> Tensor:
        ...
        update = F.linear(state[1], self.weight_td.T, None) if self.symmetric else F.linear(state[1], self.weight_bu, None)
        state[0] += update

        return state

class DualPELayer(nn.Module):
    __constants__ = ['in_features', 'out_features']
    in_features: int
    out_features: int
    weight_td: Tensor
    ...
    def __init__(self,
        ...
        factory_kwargs = {'device': device, 'dtype': dtype}
        super(DualPELayer, self).__init__()
