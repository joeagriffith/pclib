...
class FCPW(nn.Module):
    ...
    def predict(self, state):
        ...
        """
        x = self.actv_fn(state['x'].detach() / (state['e'].detach() + 1e-8))
        return F.linear(x, self.weight_td, self.bias)
    ...
    def update_x(self, state, e_below=None, temp=None):
        ...
        with torch.no_grad():
            ...
            if e_below is not None:
                ...
                dx += (update * self.d_actv_fn(state['x'] / (state['e'] + 1e-8))) / (state['e'] + 1e-8)
