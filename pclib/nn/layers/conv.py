...
class Conv2dSkip(Conv2d):
    ...
    def init_state(self, batch_size):
        return {
            'x': torch.zeros((batch_size, self.shape[0]//2, self.shape[1], self.shape[2]), device=self.device, requires_grad=True),
            'e': torch.zeros((batch_size, self.shape[0], self.shape[1], self.shape[2]), device=self.device),
        }

    # Returns a prediction of the state in the previous layer
    def predict(self, state, skip):
        x = torch.cat([state['x'], skip], dim=1)
        return self.conv_td(self.actv_fn(x))

    # Recalculates prediction-error between state and top-down prediction of it
    # With simulated annealing
    def update_e(self, state, skip, pred=None, temp=None):
        x = torch.cat([state['x'], skip], dim=1)
        if pred is not None:
            state['e'] = x - pred
