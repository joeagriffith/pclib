    ...
    def step(self, state, obs=None, y=None, temp=None):
        ...
        for i, layer in reversed(list(enumerate(self.layers))):
            ...
            if i > 0: # Bottom layer can't predict
                print(f"Getting pred from layer {i}")
...
class ConvClassifierSS(ConvClassifier):
    ...
    def _init_xs(self, state, obs=None, y=None):
        ...
        for i, state_i in enumerate(state):
            print(f"layer: {i} x mean: {state_i['x'].mean()}")
    ...
    def step(self, state, obs=None, y=None, temp=None):
        ...
        for i, layer in enumerate(self.layers):
            if isinstance(layer, Conv2d):
                ...
                if i > 0:
                    state[i]['x'].grad += -2 * torch.conv2d_input(state[i]['x'].shape, layer.conv.weight, state[i-1]['e'], layer.conv.stride, layer.conv.padding, layer.conv.dilation, layer.conv.groups)
                if i < len(self.layers) - 1:
